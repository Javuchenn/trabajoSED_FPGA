...
-- Este bloque es la máquina de estados del proyecto:
  -- ESTADOS: e_Reposo, e_WaitConfirm, e_DevolviendoDinero, e_CompraOK y e_CompraError
...
begin
  ...
  begin
    if rising_edge(clk) then
      ...
      if current_state = e_CompraOK then 
        enable_DecoderFinOK <= '1'; -- Se habilita el decoder en el modo de compra OK
        if cocacola = '1' then
          led_cocacola <= '1';
        elsif fanta = '1' then
          led_fanta <= '1';
      ...
      if current_state = e_CompraError then
        error_compra <= '1'; -- Se habilita el decoder en el modo de compra ERROR
      ...
      if current_state = e_WaitConfirm then -- Solo se cuentan las monedas en el estado de espera para confirmar
        enable_cont <= '1'; enable_DecoderCont <= '1'; -- Se habilita el decoder en el modo de "contando monedas" y tambien se habilita el contador de monedas
        led_WaitConfirm <= '1'; -- DEBUG
      else
        enable_cont <= '0'; enable_DecoderCont <= '0';
        led_WaitConfirm <= '0'; -- DEBUG
      end if;
  ...
  -- Se utiliza el temporizador cuando de un estado a otro tiene que pasar un tiempo determinado
